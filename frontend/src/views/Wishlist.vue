...
export default defineComponent({
  ...
  methods: {
    async deleteProduct(productId: string) {
      try {
        ...
        const response = await fetch(
          `https://capsule-wardrobe.onrender.com:8000/api/wishlist/${id}/${productId}  `,
          {
            method: "Delete",
            headers: {
      ...
      try {
        for (const item of this.products) {
          ...
          const detailsResponse = await fetch(
            `https://capsule-wardrobe.onrender.com:8000/api/details/${productVariantIds}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
    ...
    async updateWishlist(id: string, product: string) {
      try {
        const response = await fetch(
          `https://capsule-wardrobe.onrender.com:8000/api/wishlist/${id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
